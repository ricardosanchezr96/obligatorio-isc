#!/bin/bash
# Script que permite aprovisionar el bastión para hacer el deploy automático de
# Online Boutique
...
yum update -y 
# Instalacion de Git, para clonar el repositorio y tener los archivos necesarios
# para el deploy
yum install git -y 
...
# Se agrega al usuario ec2-user (el que utilizamos en el bastion) al grupo docker
...
# Se crea el directorio .aws (no se instal aws cli porque viene por defecto en instancias EC2)
...
# LAS CREDENCIALES DEBEN SER ELIMINADAS DEL ARCHIVO ANTES DE HACER PUSH DE LOS CAMBIOS
